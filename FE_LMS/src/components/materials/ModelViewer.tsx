import { type FC, Suspense, useRef, useLayoutEffect, useEffect, useMemo, useState, useCallback } from 'react';
...
export interface ViewerProps {
  ...
  onModelLoaded?: () => void;
  // Moon light effect props
  moonLightPosition?: number[] | null;
  moonLightStrength?: number;
  onBuildingAnimation?: () => void;
}
...
interface ModelInnerProps {
  ...
  autoRotateSpeed: number;
  onLoaded?: () => void;
  buildingAnimationProgress?: number;
}
...
const ModelInner: FC<ModelInnerProps> = ({
  ...
  const content = useMemo<THREE.Object3D | null>(() => {
    if (ext === 'glb' || ext === 'gltf') {
      if (gltf) {
        ...
        console.log('3D Model loaded. All object names:');
  ...
  useEffect(() => {
    ...
    inner.current.traverse((o: THREE.Object3D) => {
      if (o.name === "building") {
        const originalY = o.userData.originalY ?? o.position.y;
        o.userData.originalY = originalY;
        o.position.y = originalY + (buildingAnimationProgress * 1);
      }
    });
  ...
  }, [gl, enableManualRotation, camera, inner]);
  ...
  }, [gl, enableManualRotation, enableManualZoom, minZoom, maxZoom, camera, inner]);
  ...
  const handlePointerDown = useCallback((e: { object?: THREE.Object3D; stopPropagation: () => void }) => {
    const target = e.object;
    console.log('Pointer down event:', target?.name, target?.parent?.name);
    
    if (target && (target.name === "book-logo" || target.parent?.name === "book-logo")) {
      console.log('Book logo clicked! Opening modal...');
      e.stopPropagation();
      // Dispatch a custom event so parent can open modal
      window.dispatchEvent(new CustomEvent("openBookModal"));
    } else {
      console.log('Clicked object:', target?.name, 'Parent:', target?.parent?.name);
    }
  }, []);
...
const ModelViewer: FC<ViewerProps> = ({
  ...
  const handleBuildingToggle = useCallback(() => {
    ...
    // Dispatch events for building state changes
    if (willBeOpen) {
      window.dispatchEvent(new CustomEvent("buildingOpened"));
    } else {
      window.dispatchEvent(new CustomEvent("buildingClosed"));
    }
  ...
  return (
    ...
    >
      ...
      >
        {environmentPreset !== 'none' && <Environment preset={environmentPreset as 'city' | 'sunset' | 'night' | 'dawn' | 'studio' | 'apartment' | 'forest' | 'park'} background={false} />}

        <ambientLight intensity={ambientIntensity} />
        
        {/* Moon-controlled lighting */}
        {moonLightPosition ? (
          <>
            <directionalLight
              position={moonLightPosition as [number, number, number]}
